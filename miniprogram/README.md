...
3. 用户手动选择地区后的高德实时天气、今日穿搭和候选新衣的视觉混合分析；不申请定位权限。
4. 邀请制穿搭灵感广场：人工预审、单用户去重点赞、举报、作者下架和本周榜；不含评论、私信或权益兑换。
...
| 候选新衣分析 | `POST /api/candidates/:id/analyze`、`POST /api/candidates/:id/decision` | 本地已实现图片向量与标签混合分析；部署状态以 health buildId 为准。 |
...
视觉相似只用于本人衣橱重复风险，不代表品牌、货号或电商同款鉴定。支付、交易、租赁、评论、私信、人物照片和虚拟试穿均未接入。真实接口返回失败时，页面显示错误或明确降级，不得把模拟数据当作真实结果。
