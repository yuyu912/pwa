...
const statusText = {
  resolving: "正在读取公开信息",
  screenshot_required: "链接暂时无法读取，请上传截图",
  ready_to_analyze: "图片已准备好",
  analyzing: "AI 正在识别一套主穿搭",
  awaiting_confirmation: "请确认识别标签",
  ready: "已按当前衣橱完成匹配",
  failed: "本次识别未完成，可补充截图重试"
};

const mimeFromPath = (path) => /\.png(?:$|\?)/i.test(path) ? "image/png"
...
Page({
  ...
  async loadHistory() {
    try {
      const result = await api.listInspirations();
  ...
  setRecord(record) {
    ...
    this.setData({
      record: record ? {
        ...
        fallbackNote: analysisFallback
          ? "系统已经自动重试一次。平台公开图片可能不是笔记主图，上传的原笔记截图只保存在你的私密历史中。"
          : platformFallback
            ? "平台可能要求登录、验证码或限制公开访问。截图只保存在你的私密历史中。"
            : "系统已经自动重试一次，但截图中仍未找到清晰完整的核心穿搭。请尽量保留上装、下装或连衣裙的完整轮廓。"
      } : null,
      editSlots: editableSlots(record),
      errorText: ""
    });
  },
  ...
  async startLink() {
    ...
    try {
      ...
      this.setData({ errorText: error.message || "链接处理失败，请改用截图。" });
    } finally {
      this.setData({ busy: false, progressText: "" });
      this.loadHistory();
    }
  },
  ...
  chooseScreenshot() {
    ...
    wx.chooseMedia({
      ...
      success: ({ tempFiles }) => this.uploadScreenshot(tempFiles[0])
    });
  },
  ...
  async uploadScreenshot(file) {
    ...
    try {
      ...
      let upload;
      if (record && ["screenshot_required", "failed"].includes(record.status)) {
        upload = await api.createInspirationScreenshotUpload(record.id, { mimeType, size: file.size });
      } else {
        const created = await api.createInspiration({ sourceType: "user_screenshot", mimeType, idempotencyKey: idempotencyKey("shot") });
      ...
      this.setData({ errorText: error.message || "截图处理失败，请稍后重试。" });
    } finally {
      this.setData({ busy: false, progressText: "" });
      this.loadHistory();
    }
  ...
  async analyzeRecord(id) {
    ...
    const record = await api.analyzeInspiration(id);
    this.setRecord(record);
  },
  ...
  onSlotInput(event) {
    ...
    this.setData({ [`editSlots[${index}].${field}`]: event.detail.value });
  },
  ...
  async confirmTags() {
    ...
    try {
      const record = await api.confirmInspiration(this.data.record.id, { summary: this.data.record.summary, slots });
      this.setRecord(record);
      wx.showToast({ title: "匹配完成", icon: "success" });
    } catch (error) {
      this.setData({ errorText: error.message || "标签确认失败。" });
    } finally {
      this.setData({ busy: false, progressText: "" });
      this.loadHistory();
    }
  ...
  deleteHistory(event) {
    ...
    wx.showModal({
      title: "删除这条私密灵感？",
      content: "记录和你上传的私密截图将一并删除，无法恢复。",
      confirmColor: "#a65e58",
      success: async ({ confirm }) => {
        if (!confirm) return;
        try {
          await api.deleteInspiration(id);
          if (this.data.record && this.data.record.id === id) this.setData({ record: null, editSlots: [] });
